    ...
    >
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-stretch">
              ...
              return (
                <button
                  ...
                  className={`p-5 rounded-2xl border text-left transition-all duration-350 cursor-pointer active:scale-[0.99] relative overflow-hidden flex justify-between items-center ${
                    isActive
                      ? "bg-gradient-to-r from-violet-600/10 to-fuchsia-600/10 border-violet-500/80 shadow-md"
                      : "bg-slate-50 dark:bg-slate-900/60 border-slate-200 dark:border-slate-850 hover:border-slate-450 dark:hover:border-slate-700"
                  }`}
                >
                  <div className="space-y-1">
                    <div className="font-extrabold text-sm tracking-tight text-slate-950 dark:text-white flex items-center gap-2">
                      <Briefcase size={16} className={isActive ? "text-violet-500" : "text-slate-400"} />
                      {job.company}
                    </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <ChevronRight size={16} className={`transition-transform duration-300 ${isActive ? "rotate-90 text-violet-500" : "text-slate-400"}`} />
                  </div>

                  {isActive && (
                    <motion.div
                      layoutId="activeGlow"
                      className="absolute top-0 bottom-0 left-0 w-1 bg-violet-600"
          ...
          <div className="lg:col-span-8 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-850 rounded-3xl p-6 md:p-8 flex flex-col justify-between relative overflow-hidden shadow-sm">
            {/* Subtle glow background */}
            <div className="absolute top-[-20%] right-[-10%] w-[300px] h-[300px] rounded-full bg-violet-600/5 blur-[80px] pointer-events-none" />
              ...
              >
                ...
                <div className="border-b border-slate-200 dark:border-slate-800 pb-5">
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    ...
                    <div className="flex items-center gap-2 text-xs font-semibold text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-950/60 border border-slate-200 dark:border-slate-800/80 px-3 py-1.5 rounded-full shadow-sm">
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-3">
                    {activeJob.responsibilities.map((resp, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm text-slate-650 dark:text-slate-400 leading-relaxed">
                ...
                <div className="space-y-3">
                  ...
                  <ul className="space-y-3">
                    {activeJob.achievements.map((ach, i) => (
                      <li key={i} className="flex items-start gap-3 text-sm text-slate-650 dark:text-slate-400 leading-relaxed bg-white dark:bg-slate-950/40 p-3 rounded-xl border border-slate-200 dark:border-slate-800/80">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {activeJob.technologies.map((tech) => (
                      <span
                        key={tech}
                        className="px-3 py-1 text-xs font-semibold rounded-lg bg-slate-200 dark:bg-slate-800 text-slate-800 dark:text-slate-350 border border-slate-300 dark:border-slate-750"
                      >
                        {tech}
                      </span>
                    ))}
